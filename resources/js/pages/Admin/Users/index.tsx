...
export default function UsersIndex() {
    ...
    const assignRole = async (userId: number, role: string) => {
        setSaving(userId);
        try {
            const { data } = await axios.put<UserRow>(`/api/users/${userId}/roles`, {
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <p className="text-3xl font-bold">
                                {loading ? '—' : users.filter((u) => u.roles.length === 0).length}
                            </p>
                            <p className="mt-1 text-xs text-muted-foreground">Need assignment</p>
                        </CardContent>
                    </Card>
                </div>
                            ...
                            <tbody className="divide-y">
                                {filtered.map((user) => (
                                    <tr key={user.id} className="hover:bg-muted/20 transition-colors">
                                        ...
                                        <td className="px-4 py-3">
                                            <Select
                                                value={user.roles[0] ?? '__none__'}
                                                onValueChange={(val) => assignRole(user.id, val)}
